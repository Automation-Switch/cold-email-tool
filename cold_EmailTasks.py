...
class coldEmailTasks():
    ...
    def subniche(self, agent, industry, sender, briefDes, offer_pdf, offer_link):
        return Task(description=dedent(f""" 
              Identify the companies subniche within {industry} specific to its industry and the services that {sender} that will be beneficial to the job titiles within the various companies. Using information provided by a business analyst, 
              identify specific companies and subniches of {industry} and the broader industry at large.
              This task involves analyzing the company's operations, comparing it with the broader industry, 
              and using business analyst insights to pinpoint specialized areas. 
    ...
    def profile(self, agent, industry, sender, briefDes, offer_pdf, offer_link):
        return Task(description=dedent(f"""
              Identify and list the Job Titles within {industry} that are responsible for or will benefit from {offer_link}, {briefDes} or {offer_pdf}. Using the information provided by the Business Portfolio Analyst, identify the Job Titles' supervisors of the subniches.
              ...
              This analysis should provide a detailed overview of the organizational structure, including the key job titles, their roles, and their supervisors. Focus on responsibilities, potential benefits from the offer, 
              and the overall hierarchy within the company.
    ...
    def painPoints(self, agent, industry, sender, briefDes, offer_pdf, offer_link):
        return Task(description=dedent(f"""
              ...
              Your final answer MUST be a complete expanded report, 
              formatted as markdown, encompassing each step of the process, detailed explanations of pain points, their rankings, 
